...
class PHPReport {
    ...
    public function render($type='html',$filename='')
    {
		...
		elseif(strtolower($type)=='excel20034')
			return $this->renderXlscron($filename);
    ...
    /**
     * Renders report as a XLS file for cron
	 */
	private function renderXlscron($filename)
    {
       // header('Content-Type: application/vnd.ms-excel');
	//	header('Content-Disposition: attachment;filename="'.$filename.'.xls"');
	//	header('Cache-Control: max-age=0');
...
//echo '<pre>'; print_r(); exit;
        $this->objWriter->save(WWW_ROOT.'files/'.$filename.'.xls');
		unset($this->objWriter);
		unset($this->objWorksheet);
		unset($this->objReader);
		unset($this->objPHPExcel);
